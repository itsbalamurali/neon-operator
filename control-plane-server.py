...
def attach_hook(request: AttachHookRequest):
    ...
    #Check if tenant exists with tenant id i.e. request.tenant_id if not create it with gen=0 with page server id i.e. request.node_id and save it
    if request.node_id != "":
        print(f'tenant_id: {request.tenant_id}, ps_id: {request.node_id}, generation: 1, issuing')
    elif tenant_state.pageserver != "":
        print(f'tenant_id: {request.tenant_id}, ps_id: {tenant_state.pageserver}, generation: {tenant_state.generation}, dropping')
    else:
